...
internal class CellDtoTest {
    ...
    fun closed() {
        assertAll(
            { assertThat(CellDto(cell).toString()).isEqualTo("C") },
    ...
    fun open() {
        ...
        cell.open()
        assertAll(
            { assertThat(CellDto(cell).toString()).isEqualTo("1") },
            { assertThat(CellDto(cell).mineNumber()).isEqualTo("1") }
        )
    }

    @DisplayName("Cell 이 mine 이라면, mineNumber 가 * 로 렌더링이 되어야 하며 열리면 게임이 종료되어야 한다.")
    @Test
    fun mine() {
