...
class BoardTest {
    ...
    fun `사용자에게 입력받은 개수만큼 지뢰를 생성한다`(mineCount: Int) {
        val board = BoardGenerator.getOrNull(10, 10, mineCount)!!.execute()
    ...
    fun `지뢰가 아닐 경우 주변 8개 사각형에 포함된 지뢰의 개수를 표시한다`(mineCount: Int) {
        val board = BoardGenerator.getOrNull(10, 5, mineCount)!!.execute()
    ...
    fun `지뢰를 선택하면 패배한다`(boardSize: Int) {
        val board = BoardGenerator.getOrNull(boardSize, boardSize, boardSize * boardSize)!!.execute()
    ...
    fun `이미 열었던 블록은 다시 열 수 없다`(boardSize: Int) {
        val board = BoardGenerator.getOrNull(boardSize, boardSize, boardSize * boardSize - 2)!!.execute()
    ...
    fun `지뢰가 아닌 모든 블록을 열면 승리한다`(boardSize: Int) {
        val board = BoardGenerator.getOrNull(boardSize, boardSize, boardSize * boardSize - 1)!!.execute()
