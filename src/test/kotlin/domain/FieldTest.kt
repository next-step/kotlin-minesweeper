...
internal class FieldTest {
    ...
    internal fun `지뢰가 아닌 땅을 열면, 누른 땅과 지뢰가 없는 인접한 칸이 모두 열린다`() {
        ...
        // then
        assertAll(
            { assertThat((field.rows.items[0].cells[0] as Land).isOpened()).isFalse },
            { assertThat((field.rows.items[1].cells[0] as Land).isOpened()).isTrue },
            { assertThat((field.rows.items[2].cells[0] as Land).isOpened()).isTrue },
            { assertThat((field.rows.items[2].cells[1] as Land).isOpened()).isTrue },
            { assertThat((field.rows.items[3].cells[0] as Land).isOpened()).isTrue },
            { assertThat((field.rows.items[3].cells[1] as Land).isOpened()).isTrue },
            { assertThat((field.rows.items[3].cells[2] as Land).isOpened()).isTrue },
            { assertThat((field.rows.items[3].cells[3] as Land).isOpened()).isFalse },
        )
    }
}
