...
class MineFieldTest {
    ...
    fun `x, y(좌표)값이 주어 졌을때 해당 위치 Slot이 Mine인지 확인 할 수 있다`() {
        val mineField = MineField.createByIndexs(setOf(Point(1, 0)), FieldSize(5, 1))
    ...
    fun `x, y(좌표)값이 주어 졌을때 해당 위치 Slot의 Check 여부를 확인 할 수 있다`() {
        val mineField = MineField.createByIndexs(setOf(), FieldSize(5, 1))
        mineField.changeChecked(Point(4, 0))
