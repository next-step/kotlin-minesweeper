        ...
        context("생성") {
            it("보드 크기보다 지뢰 개수가 더 많으면 예외 발생") {
                assertThrows<IllegalArgumentException> {
                    MineBoard.of(board, MinePoints.of((1..101).map { Point(it, 0) }.toSet()))
                }.shouldHaveMessage("지뢰 개수는 100개보다 많을 수 없습니다.")
            }

            it("지뢰 위치 y가 보드 최대 높이 이상이면 예외 발생") {
                assertThrows<IllegalArgumentException> {
                    MineBoard.of(board, MinePoints.of(setOf(Point(0, 10))))
                }.shouldHaveMessage("지뢰 y 위치는 10 보다 작아야 합니다.")
            }

            it("지뢰 위치 x가 보드 최대 너비 이상이면 예외 발생") {
                assertThrows<IllegalArgumentException> {
                    MineBoard.of(board, MinePoints.of(setOf(Point(10, 0))))
                }.shouldHaveMessage("지뢰 x 위치는 10 보다 작아야 합니다.")
            }
        }

        context("method") {
            val mineBoard = MineBoard.of(board, MinePoints.of(setOf(Point(0, 0))))

            it("지뢰가 있는 위치이면 true") {
                mineBoard.isMineAt(Point(0, 0)) shouldBe true
            }
            it("지뢰가 없는 위치이면 false") {
                mineBoard.isMineAt(Point(1, 0)) shouldBe false
            }
            it("높이 제공") {
