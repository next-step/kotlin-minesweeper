...
class MineInstallationTest : StringSpec({
    ...
    "지뢰 개수, 마크, 포지션 선택 전략으로 생성" {
        MineInstallation(1, MineMark.MINE, ZERO_POSITION_SELECTOR)
    }
        ...
        listOf(Int.MIN_VALUE, -1)
            .forAll {
                shouldThrowExactly<IllegalArgumentException> {
                    MineInstallation(it, MineMark.MINE, ZERO_POSITION_SELECTOR)
                }
            }
    }
    ...
    "지뢰 설치 개수는 주어진 지뢰 보드 사이즈보다 작아야 함" {
        // given
        val maxCountMineInstallation = MineInstallation(Int.MAX_VALUE, MineMark.MINE, ZERO_POSITION_SELECTOR)
        // when & then
        shouldThrowExactly<IllegalArgumentException> {
    ...
    "주어지는 포지션은 메인 보드에 존재해야 함" {
        ...
        val maxPositionInstallation =
            MineInstallation(1, MineMark.MINE) { _, _ -> Position(Int.MAX_VALUE, Int.MAX_VALUE) }
        // when & then
