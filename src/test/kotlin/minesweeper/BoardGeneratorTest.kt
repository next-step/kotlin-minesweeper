...
class BoardGeneratorTest : DescribeSpec({
    ...
    describe("generate") {
        context("높이와 너비가 주어지면") {
            it("해당 크기의 보드를 생성한다.") {
                val board = BoardGenerator().generate(BoardSize(2, 2), MineCount(1))
        ...
        context("크기보다 지뢰의 개수가 많으면") {
            it("Invalid 상태를 반환한다.") {
                shouldThrow<IllegalArgumentException> {
                    BoardGenerator().generate(BoardSize(1, 1), MineCount(10))
                }
            }
        }
        ...
        context("지뢰의 개수가 만큼") {
            it("지뢰를 보드에 추가한다.") {
                val board = BoardGenerator().generate(BoardSize(2, 2), MineCount(2))
        ...
        context("지정된 위치에") {
            it("지뢰를 배치한다.") {
                ...
                val board = BoardGenerator(mineSpawner).generate(BoardSize(2, 2), MineCount(2))
