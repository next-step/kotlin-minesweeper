...
class LandTest {
    ...
    fun `땅의 넓이를 넘는 지뢰를 심을 수 없다`() {
        assertThrows<IllegalStateException> {
            ResultView.showLand(Land.from(LandSize(10, 10), 101, GameApp::generateMines))
        }
    }
    ...
    fun `10x10 크기의 땅을 만든다`() {
        assertDoesNotThrow {
            ResultView.showLand(Land.from(LandSize(10, 10), 10, GameApp::generateMines))
        }
    }
    ...
    fun `생성된 지뢰 갯수를 확인한다`(count: Int) {
        val land = Land.from(LandSize(10, 10), count, GameApp::generateMines)
        val actualCount: Int =
