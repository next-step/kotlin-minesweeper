...
class RandomMineFieldGeneratorTest {
    ...
    fun `너비가 1미만이면 예외 발생`() {
        // when, then
        val exception = assertThrows<IllegalArgumentException> { RandomMineFieldGenerator.generate(0, 2, 1) }
    ...
    fun `높이가 1미만이면 예외 발생`() {
        // when, then
        val exception = assertThrows<IllegalArgumentException> { RandomMineFieldGenerator.generate(2, 0, 1) }
    ...
    fun `지뢰 개수가 1개 미만이면 예외 발생`() {
        // when, then
        val exception = assertThrows<IllegalArgumentException> { RandomMineFieldGenerator.generate(2, 2, 0) }
    ...
    fun `지뢰 개수가 지뢰판 사이즈를 초과하면 예외 발생`() {
        // when, then
        val exception = assertThrows<IllegalArgumentException> { RandomMineFieldGenerator.generate(2, 2, 5) }
