...
internal class RemainderPlantStrategyTest {
    ...
    fun `원하는 만큼의 지뢰가 나왔는지 테스트 한다`() {
        Assertions.assertThat(RemainderPlantStrategy().createMines(width = 5, height = 5, mineCount = 10).size)
            .isEqualTo(10)
    }

    @Test
    fun `3 * 3의 배열에서 5개를 심으면 아래와 같은 모양으로 지뢰를 심는다`() {
        Assertions.assertThat(RemainderPlantStrategy().createMines(width = 3, height = 3, mineCount = 5))
