...
class TilesTest {
    ...
    fun `Tiles - 중복 타일 생성에 대한 예외처리 테스트`() {
        // given, when, then
        assertThatThrownBy { Tiles(NotChecked(0, 0, false), NotChecked(0, 0, false)) }
    ...
    fun `Tiles - 타일 확인 테스트`() {
        // given
        val given = Tiles(NotChecked(0, 0, false), Mine(0, 1))
        val expected = Tiles(NotMines(0, 0, Marking.ONE), Mine(0, 1))
