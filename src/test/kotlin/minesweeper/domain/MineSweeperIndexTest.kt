...
class MineSweeperIndexTest : BehaviorSpec({
    ...
    given("좌표(1,1)와 지뢰여부 상태가 주어졌을 때") {
        ...
        When("지뢰 찾기 인덱스를 생성하면") {
            val mineSweeperIndex = MineSweeperIndex(position, mineStatus)
            Then("좌표와 상태가 같다.") {
    ...
    given("좌표 (1,1)인 지뢰찾기 인덱스가 주어졌을때") {
        val mineSweeperIndex = MineSweeperIndex(1, 1, MineStatus.NOT_MINE)
        When("열림 상태로 변경하면") {
    ...
    given("지뢰 인덱스(3x3)리스트가 주어지고 (3,3)가 지뢰일때") {
        ...
        When("(2,2)좌표의 지뢰의 개수를 구하면") {
            val result = MineSweeperIndex(2, 2, MineStatus.NOT_MINE).mineCount(mineSweeperIndexes)
            Then("지뢰의 개수는 1이다.") {
                result shouldBe 1
            }
        }
    ...
    given("지뢰 인덱스(1, 1) 지뢰일때") {
        val mineSweeperIndex = MineSweeperIndex(1, 1, MineStatus.MINE)
