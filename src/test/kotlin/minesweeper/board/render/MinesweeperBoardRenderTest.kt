    ...
    Given("지뢰들이 존재하고") {
        ...
        When("게임판 크기와 셀 값이 주어진다면") {
            ...
            Then("지뢰찾기 게임판을 생성한다.") {
                ...
                val expect = MinesweeperGameBoard(
                    listOf(
                        listOf(
        ...
        When("게임판 크기와 셀 값이 주어진다면2") {
            val element = BoardElement(3, 3)

            Then("지뢰찾기 게임판을 생성한다.2") {
                val actual = MinesweeperBoardRender(mines2)(element, '0')
                val expect = MinesweeperGameBoard(
                    listOf(
                        listOf(
                            Cell(Position(0, 0), CellType.NORMAL, '0'),
                            Cell(Position(0, 1), CellType.NORMAL, '0'),
                            Cell(Position(0, 2), CellType.NORMAL, '0')
                        ),
                        listOf(
                            Cell(Position(1, 0), CellType.NORMAL, '1'),
                            Cell(Position(1, 1), CellType.NORMAL, '1'),
                            Cell(Position(1, 2), CellType.NORMAL, '1')
                        ),
                        listOf(
                            Cell(Position(2, 0), CellType.NORMAL, '1'),
                            Cell(Position(2, 1), CellType.MINE, '0'),
                            Cell(Position(2, 2), CellType.NORMAL, '1')
                        )
                    )
                )
                actual shouldBe expect
