...
class AdjacentMineCounter : MineCounter {
    ...
    private fun Position.getNeighborPositions(boardHeight: Int, boardWidth: Int): List<Position> {
        return (-1..1).flatMap { dx ->
            (-1..1).mapNotNull { dy ->
                Position(boardHeight + dx, boardWidth + dy).takeIf { it.isNeighborOf(this) && it.isWithinBounds(boardHeight, boardWidth) }
            }
        }
    }

    private fun Position.isNeighborOf(other: Position): Boolean = this != other
