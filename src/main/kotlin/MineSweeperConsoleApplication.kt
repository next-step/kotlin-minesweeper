fun main() {
    ...
    val board = MineSweeperBoard.of(
        height = inputHeight.toInt(),
        width = inputWidth.toInt(),
        mineCount = inputMineCount.toInt(),
    )

    repeat(inputHeight.toInt()) { height ->
        repeat(inputWidth.toInt()) {
            print("${board.field.get((it) + ((height) * inputWidth.toInt())).symbol()} ")
        }
        println()
    }
}
